...
namespace NexusDev
{
    public class ExportDaggerfallAssets_West : EditorWindow
    {
...
private const string PrefTxtPath = "NexusDev.ExportDaggerfallAssets_West.TxtPath";
private const string PrefDeleteTemps = "NexusDev.ExportDaggerfallAssets_West.DeleteTemps";
private const string PrefSingleModelTxtPath = "NexusDev.ExportDaggerfallAssets_West.SingleModelTxtPath";
private const string PrefSingleFlatTxtPath = "NexusDev.ExportDaggerfallAssets_West.SingleFlatTxtPath";
private const string PrefSingleModelBatchRangeText = "NexusDev.ExportDaggerfallAssets_West.SingleModelBatchRangeText";
private const string PrefSingleFlatBatchRangeText = "NexusDev.ExportDaggerfallAssets_West.SingleFlatBatchRangeText";
private const string PrefSingleId = "NexusDev.ExportDaggerfallAssets_West.SingleId";
private const string PrefSingleFlatId = "NexusDev.ExportDaggerfallAssets_West.SingleFlatId";
private const string PrefExportScaleMultiplier = "NexusDev.ExportDaggerfallAssets_West.ExportScaleMultiplier";
private const string PrefAtlasBatchExportedMats = "NexusDev.ExportDaggerfallAssets_West.AtlasBatchExportedMats";
private const string PrefBatchAtlasMaxSize = "NexusDev.ExportDaggerfallAssets_West.BatchAtlasMaxSize";
private const string PrefBatchRangeText = "NexusDev.ExportDaggerfallAssets_West.BatchRangeText";
private const string PrefCategoryBatchExpanded = "NexusDev.ExportDaggerfallAssets_West.CategoryBatchExpanded";
private const string PrefCategoryBatchSelected = "NexusDev.ExportDaggerfallAssets_West.CategoryBatchSelected";
private const string PrefFlatCategoryBatchExpanded = "NexusDev.ExportDaggerfallAssets_West.FlatCategoryBatchExpanded";
private const string PrefFlatCategoryBatchSelected = "NexusDev.ExportDaggerfallAssets_West.FlatCategoryBatchSelected";

private const string PrefOverwriteExistingExportAssets = "NexusDev.ExportDaggerfallAssets_West.OverwriteExistingExportAssets";
private const string PrefExportGroupedFamilyMembers = "NexusDev.ExportDaggerfallAssets_West.ExportGroupedFamilyMembers";
private const string PrefCategoryBatchFbxSubfolders = "NexusDev.ExportDaggerfallAssets_West.CategoryBatchFbxSubfolders";
private const string PrefCategoryLastDurationSecondsPrefix = "NexusDev.ExportDaggerfallAssets_West.CategoryLastDurationSeconds.";
private const string PrefFlatCategoryLastDurationSecondsPrefix = "NexusDev.ExportDaggerfallAssets_West.FlatCategoryLastDurationSeconds.";
private const string PrefMobileSpriteSortMode = "NexusDev.ExportDaggerfallAssets_West.MobileSpriteSortMode";
...
private bool singleModelSectionExpanded = true;
private bool singleFlatSectionExpanded = true;
...
#
# Expected root structure (OUTDATED, IGNOR ETHIS):
#   <Assets\Exports\DaggerfallAssets>
#       FBX\
#       _Material\
#       _TextureAtlas\
#
...
private const string MobileTownExteriorFolderAssetRelative = "Assets/Exports/DaggerfallAssets/MobileSprites/TownExterior";
private const string MobileCombatantFolderAssetRelative = "Assets/Exports/DaggerfallAssets/MobileSprites/Combatant";
...
private bool mobileIncludeMove = true;
private bool mobileIncludeIdle = true;
...
private bool mobileTownExteriorSectionExpanded = true;
private bool mobileCombatantSectionExpanded = true;
...
private readonly List<DeferredFbxMaterialRemapRequest> deferredFbxMaterialRemapRequests =
    new List<DeferredFbxMaterialRemapRequest>();

private bool ultraExportIncludeModels = true;
private bool ultraExportIncludeFlats = true;
private bool ultraExportIncludeMobileSprites = true;

private class DeferredFbxMaterialRemapRequest
{
    public string FbxAssetPath;
    public Material[] DesiredMaterials;
}

private void QueueDeferredFbxMaterialRemap(string fbxAssetPath, Material[] desiredMaterials)
{
    if (string.IsNullOrWhiteSpace(fbxAssetPath) || desiredMaterials == null || desiredMaterials.Length == 0)
        return;

    Material[] copy = new Material[desiredMaterials.Length];
    Array.Copy(desiredMaterials, copy, desiredMaterials.Length);

    deferredFbxMaterialRemapRequests.Add(new DeferredFbxMaterialRemapRequest
    {
        FbxAssetPath = fbxAssetPath,
        DesiredMaterials = copy,
    });
}

private void ProcessDeferredFbxMaterialRemaps()
{
    if (deferredFbxMaterialRemapRequests == null || deferredFbxMaterialRemapRequests.Count == 0)
        return;

    for (int i = 0; i < deferredFbxMaterialRemapRequests.Count; i++)
    {
        DeferredFbxMaterialRemapRequest request = deferredFbxMaterialRemapRequests[i];
        if (request == null || string.IsNullOrWhiteSpace(request.FbxAssetPath))
            continue;

        string remapError;
        if (!RemapImportedFbxMaterials(request.FbxAssetPath, request.DesiredMaterials, out remapError))
        {
            Debug.LogError("[NexusDev][DFU Export] Deferred FBX material remap failed for '" +
                request.FbxAssetPath + "'. " + remapError);
        }
    }

    deferredFbxMaterialRemapRequests.Clear();
}

private void DrawSharedFbxExportOptionsBlock()
{
    EditorGUILayout.LabelField("Shared FBX Batch/Single Options", EditorStyles.boldLabel);

    DrawGroupedFamilyToggleShared();
    DrawAtlasBatchToggleShared();
    DrawCategorySubfolderToggleShared();

    GUILayout.Space(6);
}

private void DrawSharedFbxExportOptionsReadOnlySummary()
{
    EditorGUILayout.LabelField("Shared FBX Options", EditorStyles.boldLabel);
    EditorGUILayout.LabelField(
        "These same options are used by both Models and Flats in the Single and Category tabs.",
        EditorStyles.wordWrappedMiniLabel);

    GUILayout.Space(2);
    EditorGUILayout.LabelField(
        "Grouped family members: " + (exportGroupedFamilyMembers ? "ON" : "OFF"),
        EditorStyles.miniLabel);
    EditorGUILayout.LabelField(
        "Atlas batch exported mats: " + (atlasBatchExportedMats ? "ON" : "OFF"),
        EditorStyles.miniLabel);
    EditorGUILayout.LabelField(
        "Category FBX subfolders: " + (categoryBatchFbxSubfolders ? "ON" : "OFF"),
        EditorStyles.miniLabel);

    GUILayout.Space(8);
}

private double GetUltraExportEstimatedSeconds(out bool hasMissing)
{
    hasMissing = false;
    double total = 0d;

    if (ultraExportIncludeModels)
    {
        bool missing;
        total += GetAllCategoryEstimatedSeconds(out missing);
        hasMissing |= missing;
    }

    if (ultraExportIncludeFlats)
    {
        bool missing;
        total += GetAllFlatCategoryEstimatedSeconds(out missing);
        hasMissing |= missing;
    }

    if (ultraExportIncludeMobileSprites)
    {
        bool missing;
        total += GetTotalEstimatedMobileExportSeconds(out missing);
        hasMissing |= missing;
    }

    return total;
}
private void ExportUltraEverything()
{
    double startSeconds = EditorApplication.timeSinceStartup;
    BeginTrackingMostRecentExportOperation();

    BeginDeferredAssetEditing();
    try
    {
        if (ultraExportIncludeModels)
            ExportAllFixedCategories(false);

        if (ultraExportIncludeFlats)
            ExportAllFlatCategories(false);
    ...
    double elapsed = EditorApplication.timeSinceStartup - startSeconds;
    Debug.Log("[NexusDev][DFU Export] ULTRA export finished in " + FormatDurationSeconds(elapsed));
}
...
private static readonly HashSet<string> ensuredFolderCache =
...
private static string BuildBatchBaseName(List<uint> ids)
{
    ...
    if (minId == maxId)
        return "DaggerfallModel_id" + minId.ToString(CultureInfo.InvariantCulture);

    return "DaggerfallModel_id" +
...
private bool TryGetAnyCategoryDisplayNameForModelId(uint modelId, out string categoryFieldName, out string displayName)
{
    ...
    for (int i = 0; i < fields.Length; i++)
    {
        ...
        if (string.Equals(field.Name, "modelGroups", StringComparison.OrdinalIgnoreCase) ||
            string.Equals(field.Name, "flatGroups", StringComparison.OrdinalIgnoreCase))
        {
            continue;
        }

        if (DoesFieldContainAnyFlatArchiveRecords(field))
            continue;

        string candidate;
        if (TryGetCategoryDisplayNameForModelId(field.Name, modelId, out candidate) &&
            !string.IsNullOrWhiteSpace(candidate))
        {
            categoryFieldName = field.Name;
            displayName = candidate;
            return true;
        }
    }
...
private string BuildInfoReadmeText(InfoDocRow[] rows)
{
    ...
    sb.AppendLine("Version 7, April 2, 2026. Made by westingtyler. Requires Unity FBX Exporter package. Tested with FBX Exporter package 4.2.1. Tested in Unity 2019.4.40f1 with Daggerfall Unity 1.1.1. Exported FBX files import directly into Blender 4.5.7 LTS. Drop this into an Editor folder in your Daggerfall Unity Project, and it should appear in the NexusDev menu at the top.");
    sb.AppendLine();
    sb.AppendLine("UV / atlas note:");
    sb.AppendLine();
    sb.AppendLine("To preserve UV textures without needing cutout shaders or separate tiling modulus shaders, so it's compatible with Blender, big tiled planes like building walls are subdivided. That means exported FBX models that would have tiling textures in classic Daggerfall are higher-poly than in the original game. That's fine for modeling reference but not 100 percent vanilla-accurate. It's a tradeoff so simple opaque atlased materials work, and this kind of tradeoff is also common in professional development.");
    sb.AppendLine();
...
public static void OpenWindow()
        {
            ExportDaggerfallAssets_West window = GetWindow<ExportDaggerfallAssets_West>("DFU Export");
...
private GUIStyle GetGroupedWarningStyle()
{
    if (cachedGroupedWarningStyle == null)
    {
        ...
        cachedGroupedWarningStyle.normal.textColor = new Color(0.60f, 1f, 0.60f, 1f);
    }

    return cachedGroupedWarningStyle;
}
private GUIStyle GetCategoryInfoMiniStyle()
{
    GUIStyle style = new GUIStyle(EditorStyles.miniLabel);
    style.normal.textColor = new Color(0.60f, 1f, 0.60f, 1f);
    return style;
}
...
private void EndDeferredAssetEditing(bool refreshAndSave)
{
    ...
    if (assetEditingDepth == 0)
    {
        ...
        if (refreshAndSave)
        {
            ...
            ProcessDeferredFbxMaterialRemaps();
...
private void DrawAtlasBatchToggleShared(string label = "Atlas Batch Exported Mats")
{
    bool newValue = EditorGUILayout.ToggleLeft(label, atlasBatchExportedMats);
    if (newValue != atlasBatchExportedMats)
    {
        atlasBatchExportedMats = newValue;
        SavePrefs();
        Repaint();
    }
}

private void DrawCategorySubfolderToggleShared(string label = "Put category batch FBX files into category subfolders")
{
    bool newValue = EditorGUILayout.ToggleLeft(label, categoryBatchFbxSubfolders);
    if (newValue != categoryBatchFbxSubfolders)
    {
        categoryBatchFbxSubfolders = newValue;
        SavePrefs();
        Repaint();
    }
}

private void ResolveModelExportNaming(uint modelId, out string exportBaseName, out string categoryFieldNameOrNull)
{
    categoryFieldNameOrNull = null;
    exportBaseName = BuildDefaultExportBaseName(modelId);
    ...
    {
        categoryFieldNameOrNull = resolvedFieldName;
        exportBaseName = BuildCategoryExportBaseName(label, modelId, resolvedFieldName);
    }
}

private void ResolveFlatExportNaming(int archive, int record, out string exportBaseName, out string categoryFieldNameOrNull)
{
    categoryFieldNameOrNull = null;
    exportBaseName = BuildDefaultExportBaseNameForFlat(archive, record);

    string resolvedFieldName;
    string label;
    if (TryGetResolvedDisplayNameForFlat(null, archive, record, out resolvedFieldName, out label) &&
        !string.IsNullOrWhiteSpace(label))
    {
        categoryFieldNameOrNull = resolvedFieldName;
        exportBaseName = BuildCategoryExportBaseNameForFlat(label, archive, record, resolvedFieldName);
    }
}

private void SaveMostRecentExportedAssetPaths(params string[] assetPaths)
{
    ...
    EditorPrefs.SetString(
        "NexusDev.ExportDaggerfallAssets_West.MostRecentExportedAssetPaths",
        string.Join("|", new List<string>(unique).ToArray()));
}

private string[] GetMostRecentExportedAssetPaths()
{
    string raw = EditorPrefs.GetString(
        "NexusDev.ExportDaggerfallAssets_West.MostRecentExportedAssetPaths",
        string.Empty);
...
private void BeginTrackingMostRecentExportOperation()
{
    EditorPrefs.SetString(
        "NexusDev.ExportDaggerfallAssets_West.MostRecentExportedAssetPaths",
        string.Empty);
...
private void AppendMostRecentExportedAssetPaths(params string[] assetPaths)
{
    HashSet<string> unique = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

    string[] existing = GetMostRecentExportedAssetPaths();
    if (existing != null)
    {
        for (int i = 0; i < existing.Length; i++)
        {
            if (!string.IsNullOrWhiteSpace(existing[i]))
                unique.Add(existing[i].Replace('\\', '/'));
        }
    }
    ...
    if (assetPaths != null)
    {
        for (int i = 0; i < assetPaths.Length; i++)
        {
            if (!string.IsNullOrWhiteSpace(assetPaths[i]))
                unique.Add(assetPaths[i].Replace('\\', '/'));
        }
    }
...
private void DeleteMostRecentExportedAssets()
{
    string[] paths = GetMostRecentExportedAssetPaths();
    if (paths == null || paths.Length == 0)
    {
        Debug.LogWarning("[NexusDev][DFU Export] No tracked recent exported assets were found.");
        DeleteKnownEmptyDuplicateExportFolders();
        AssetDatabase.Refresh();
        return;
    }
    ...
    try
    {
        for (int i = 0; i < paths.Length; i++)
        {
            ...
            {
                string abs = GetAbsolutePathFromAssetPath(path);
        ...
        AssetDatabase.Refresh();
        EditorPrefs.DeleteKey("NexusDev.ExportDaggerfallAssets_West.MostRecentExportedAssetPaths");

        Debug.Log(string.Format(
            CultureInfo.InvariantCulture,
            "[NexusDev][DFU Export] Deleted {0} tracked most-recent exported asset path(s).",
            deletedCount));
    }
    catch (Exception ex)
    {
        Debug.LogError("[NexusDev][DFU Export] Failed deleting most recent exported assets. Exception: " + ex);
    }
}
...
private void DrawSingleExportTab()
{
    ...
    DrawSingleTabIntro();
    DrawSharedFbxExportOptionsBlock();

    bool newModelFoldout = EditorGUILayout.Foldout(singleModelSectionExpanded, "Models FBX", true);
    if (newModelFoldout != singleModelSectionExpanded)
    {
        singleModelSectionExpanded = newModelFoldout;
        SavePrefs();
    }

    if (singleModelSectionExpanded)
    {
    ...
    bool newFlatFoldout = EditorGUILayout.Foldout(singleFlatSectionExpanded, "Flats FBX", true);
    if (newFlatFoldout != singleFlatSectionExpanded)
    {
        singleFlatSectionExpanded = newFlatFoldout;
        SavePrefs();
    }

    if (singleFlatSectionExpanded)
    {
        EditorGUI.indentLevel++;
        ...
        EditorGUILayout.LabelField("Flat Archive.Record List (not reliable, consider category export instead)", EditorStyles.boldLabel);
...
private void DrawBatchExportTab()
{
    ...
    bool newModelRootExpanded = EditorGUILayout.Foldout(categoryBatchExpanded, "Model Categories", true);
    if (newModelRootExpanded != categoryBatchExpanded)
    {
        categoryBatchExpanded = newModelRootExpanded;
        SavePrefs();
    }
    ...
    if (categoryBatchExpanded)
    {
        ...
        for (int i = 0; i < selectedCategoryBatchSources.Count; i++)
        {
            ...
            EditorGUILayout.LabelField(rowInfo, GetCategoryInfoMiniStyle(), GUILayout.Width(230));
    ...
    bool newFlatRootExpanded = EditorGUILayout.Foldout(flatCategoryBatchExpanded, "Flat Categories", true);
    if (newFlatRootExpanded != flatCategoryBatchExpanded)
    {
        flatCategoryBatchExpanded = newFlatRootExpanded;
        SavePrefs();
    }
...
private void DrawSettingsPathsTab()
{
    ...
    DrawSharedFbxExportOptionsBlock();
    DrawSharedFbxExportOptionsReadOnlySummary();
...
private void DrawDebugTab()
{
    ...
    if (GUILayout.Button("Delete Last Operation's Exported Assets", GUILayout.Height(24), GUILayout.ExpandWidth(false)))
    {
        bool confirmed = EditorUtility.DisplayDialog(
            "Delete Last Operation",
            "Delete the most recently tracked exported assets?\n\nThis cannot be undone.",
            "Delete Last Operation",
            "Cancel");

        if (confirmed)
            DeleteMostRecentExportedAssets();
    }

    GUILayout.Space(10);

    if (GUILayout.Button("ULTRA DELETE ALL Previously-Exported Models, Flats, and MobileSprites", GUILayout.Height(28), GUILayout.ExpandWidth(false)))
    {
        bool confirmed = EditorUtility.DisplayDialog(
            "ULTRA DELETE ALL Exported Assets",
            "Delete ALL previously-exported model FBX files, flat FBX files, atlas textures, materials, temp export folders, and ALL MobileSprites folders/files?\n\nThis cannot be undone.",
            "ULTRA DELETE ALL",
            "Cancel");

        if (confirmed)
            DeleteAllExportedEverything();
    }

    GUILayout.Space(12);

    EditorGUILayout.LabelField("ULTRA Export", EditorStyles.boldLabel);
    ultraExportIncludeModels = EditorGUILayout.ToggleLeft("Include ALL Model Categories", ultraExportIncludeModels);
    ultraExportIncludeFlats = EditorGUILayout.ToggleLeft("Include ALL Flat Categories", ultraExportIncludeFlats);
    ultraExportIncludeMobileSprites = EditorGUILayout.ToggleLeft("Include ALL MobileSprites", ultraExportIncludeMobileSprites);

    bool ultraHasMissingEstimate;
    double ultraEstimateSeconds = GetUltraExportEstimatedSeconds(out ultraHasMissingEstimate);

    EditorGUILayout.LabelField(
        "Ultra ETA: " + FormatDurationSecondsWithPlus(ultraEstimateSeconds, ultraHasMissingEstimate),
        EditorStyles.miniBoldLabel);

    if (ultraHasMissingEstimate)
        EditorGUILayout.LabelField("+ means one or more included parts have no saved timing yet.", EditorStyles.miniLabel);

    if (GUILayout.Button("ULTRA Export Everything In Sequence", GUILayout.Height(30), GUILayout.ExpandWidth(false)))
    {
        if (ConfirmBulkExportWithEstimate(
            "ULTRA Export",
            "Run the full sequential export for the enabled sections?",
            ultraEstimateSeconds,
            ultraHasMissingEstimate,
            true))
        {
            SavePrefs();
            ExportUltraEverything();
        }
    }
    ...
    if (GUILayout.Button("Delete ALL Previously-Exported MobileSprites", GUILayout.Height(24), GUILayout.ExpandWidth(false)))
    {
        bool confirmed = EditorUtility.DisplayDialog(
            ...
            "Delete the entire previously-exported MobileSprites folder contents?\n\nThis will remove all TownExterior and Combatant mobile PNGs and JSON manifests.\n\nThis cannot be undone.",
            "Delete All MobileSprites",
            "Cancel");
...
private void DrawInfoTab()
{
    ...
    InfoDocRow[] rows = new InfoDocRow[]
    {
        ...
        InfoDocRow.MakeScript(
            "ExportDaggerfallAssets_West.cs - parses names like TEXTURE_069_Index_3_ and TEXTURE.112 [Index=1] when resolving texture identity.",
            "ExportDaggerfallAssets_West"),
        ...
        InfoDocRow.MakeScript(
            ...
            "TextureFile"),
        InfoDocRow.MakeScript(
            "ExportDaggerfallAssets_West.cs - contains the Town NPC archive mappings used by this exporter.",
            "ExportDaggerfallAssets_West"),

        InfoDocRow.MakeHeader("Name cleanup / final export naming"),
        InfoDocRow.MakeScript(
            "ExportDaggerfallAssets_West.cs - applies replacement tables, overrides, and category-based naming cleanup.",
            "ExportDaggerfallAssets_West"),

        InfoDocRow.MakeHeader("Shared-atlas batch export"),
        InfoDocRow.MakeScript(
            "ExportDaggerfallAssets_West.cs - packs readable DFU textures into atlas pages and remaps mesh UVs/materials.",
            "ExportDaggerfallAssets_West"),
...
private void DrawMobileSpritesTab()
{
    ...
    EditorGUILayout.HelpBox(
        "Exports organized PNG frame sets plus one JSON manifest per entity folder. TownExterior uses semantic archive mappings. Combatants are driven from EnemyBasics plus the guard set.",
        MessageType.None);
    GUILayout.Space(6);
    ...
    EditorGUILayout.LabelField("Include Anims", EditorStyles.boldLabel);
    mobileIncludeMove = EditorGUILayout.ToggleLeft("Include Move", mobileIncludeMove);
    mobileIncludeIdle = EditorGUILayout.ToggleLeft("Include Idle", mobileIncludeIdle);

    GUILayout.Space(2);
    EditorGUILayout.LabelField("Combatant-specific", EditorStyles.miniBoldLabel);
    ...
    mobileTownExteriorSectionExpanded = EditorGUILayout.Foldout(mobileTownExteriorSectionExpanded, "TownExterior", true);
    if (mobileTownExteriorSectionExpanded)
    {
        EditorGUI.indentLevel++;
        ...
        {
            EditorGUILayout.HelpBox("TownExterior mobile list returned no entries.", MessageType.Warning);
        }
        else
        {
            ...
            sortedTownNpcIndex = EditorGUILayout.Popup("TownExterior", sortedTownNpcIndex, townNpcLabels);
            ...
            EditorGUILayout.LabelField(
                "Folder Preview",
                MobileTownExteriorFolderAssetRelative + "/" + previewEntityName + "/",
                EditorStyles.miniLabel);
            ...
            if (GUILayout.Button("Export Current TownExterior Set", GUILayout.Height(28), GUILayout.ExpandWidth(false)))
            {
                if (ConfirmBulkExportWithEstimate(
                    "Export Current TownExterior Set",
                    "Export the current TownExterior mobile sprite set?",
            ...
            if (GUILayout.Button("Export ALL TownExterior Sets", GUILayout.Height(28), GUILayout.ExpandWidth(false)))
            {
                if (ConfirmBulkExportWithEstimate(
                    "Export ALL TownExterior Sets",
                    "Export all TownExterior mobile sprite sets?",
            ...
            if (GUILayout.Button("Delete Current TownExterior Set", GUILayout.Height(24), GUILayout.ExpandWidth(false)))
            {
                bool confirmed = EditorUtility.DisplayDialog(
                    "Delete Previous Export of Currently-Selected TownExterior Set",
                    "Delete previous export of the currently selected TownExterior mobile sprite folder?\n\n" +
                    previewEntityName + "\n\n" +
                    "This cannot be undone.",
                    "Delete Current TownExterior Set",
                    "Cancel");
            ...
            if (GUILayout.Button("Delete Previous Exports of ALL TownExterior Sets", GUILayout.Height(24), GUILayout.ExpandWidth(false)))
            {
                bool confirmed = EditorUtility.DisplayDialog(
                    "Delete Previous Exports of ALL TownExterior Sets",
                    "Delete all previously-exported TownExterior mobile sprite folders and manifests?\n\nThis cannot be undone.",
                    "Delete All TownExterior Sets",
                    "Cancel");
    ...
    mobileCombatantSectionExpanded = EditorGUILayout.Foldout(mobileCombatantSectionExpanded, "Combatant", true);
    if (mobileCombatantSectionExpanded)
    {
        EditorGUI.indentLevel++;

        EditorGUILayout.LabelField("Combatant Set", EditorStyles.boldLabel);
        ...
        {
            EditorGUILayout.HelpBox("EnemyBasics returned no combatant mobile entries.", MessageType.Warning);
        }
        else
        {
            ...
            sortedPopupIndex = EditorGUILayout.Popup("Combatant", sortedPopupIndex, enemyLabels);
            ...
            EditorGUILayout.LabelField(
                "Folder Preview",
                MobileCombatantFolderAssetRelative + "/" + previewEnemyFolderLabel + "/",
                EditorStyles.miniLabel);
            ...
            if (GUILayout.Button("Export Current Combatant Set", GUILayout.Height(28), GUILayout.ExpandWidth(false)))
            {
                if (ConfirmBulkExportWithEstimate(
                    "Export Current Combatant Set",
                    "Export the current combatant mobile sprite set?",
            ...
            if (GUILayout.Button("Export ALL Combatant Sets", GUILayout.Height(28), GUILayout.ExpandWidth(false)))
            {
                if (ConfirmBulkExportWithEstimate(
                    "Export ALL Combatant Sets",
                    "Export all combatant mobile sprite sets?",
            ...
            if (GUILayout.Button("Delete Current Combatant Set", GUILayout.Height(24), GUILayout.ExpandWidth(false)))
            {
                bool confirmed = EditorUtility.DisplayDialog(
                    "Delete Current Combatant Set",
                    "Delete the currently selected combatant mobile sprite folder or folders?\n\n" +
...
{
    ExportAllTownNpcMobileSets(true);
}

private void ExportAllTownNpcMobileSets(bool beginTrackingMostRecentOperation)
{
    if (!ValidateBeforeTownNpcMobileExport())
        return;
    ...
    if (beginTrackingMostRecentOperation)
        BeginTrackingMostRecentExportOperation();
...
{
    ExportAllEnemyMobileSets(true);
}

private void ExportAllEnemyMobileSets(bool beginTrackingMostRecentOperation)
{
    ...
    try
    {
        EditorUtility.DisplayProgressBar(
            "DFU Export",
            "Exporting combatant mobile sprites: GuardMale00",
            total > 0 ? 1f / total : 0f);

        ExportEnemyMobileSetByIndex(-1);
        ...
        for (int i = 0; i < enemies.Length; i++)
        {
            ...
            EditorUtility.DisplayProgressBar(
                "DFU Export",
                "Exporting combatant mobile sprites: " + label,
                total > 0 ? (float)(i + 2) / total : 0f);
...
{
    if (!ValidateBeforeFullMobileSpriteExport())
        return;
    ...
    try
    {
        ...
        ExportAllTownNpcMobileSets(false);
        ExportAllEnemyMobileSets(false);
    ...
    bool hasRangedAttack2)
{
    MobileSpriteAnimSpec[] specs = BuildEnemyAnimSpecs(enemyId, isFemaleVariant, hasIdle, hasRangedAttack1, hasRangedAttack2);
       ...
       ExportGenericMobileSpriteSet(
        MobileCombatantFolderAssetRelative,
        "CombatantMobile",
        "Combatant",
        entityName,
        archive,
...
private void DeleteCurrentTownNpcMobileSet()
{
    ...
    try
    {
                bool deleted = DeleteMobileSpriteEntityFolder(MobileTownExteriorFolderAssetRelative, entityName);
        AssetDatabase.Refresh();
...
private void DeleteAllTownNpcMobileSprites()
{
    ...
    try
    {
                deletedCount += DeleteAllFilesInAssetFolder(MobileTownExteriorFolderAssetRelative);
        AssetDatabase.Refresh();

            EnsureFolderExists(MobileSpritesFolderAssetRelative);
        EnsureFolderExists(MobileTownExteriorFolderAssetRelative);
        EnsureFolderExists(MobileCombatantFolderAssetRelative);
        AssetDatabase.Refresh();
...
private void DeleteCurrentEnemyMobileSet()
{
    ...
    try
    {
        ...
        {
                       if (DeleteMobileSpriteEntityFolder(MobileCombatantFolderAssetRelative, folderNames[i]))
                deletedCount++;
        }
...
private List<EnemyMobileListEntry> BuildEnemyMobileListEntriesUncached()
{
    ...
    EnemyMobileListEntry guardEntry = new EnemyMobileListEntry();
    guardEntry.OriginalIndex = -1;
    guardEntry.EnemyId = 399;
    guardEntry.BaseName = "GuardMale00";
    guardEntry.Label = "GuardMale00 [id 399 | A:399]";
...
private bool ValidateBeforeTownNpcMobileExport()
{
    ...
    if (!mobileIncludeMove && !mobileIncludeIdle)
    {
    ...
    bool isGuard)
{
    ...
    string townNpcRootAbs = GetAbsolutePathFromAssetPath(MobileTownExteriorFolderAssetRelative);
    ...
    manifest.exportType = "TownExteriorMobile";
    ...
    for (int i = 0; i < specs.Length; i++)
    {
       ...
       if (spec.AnimationName == "Move" && !mobileIncludeMove)
            continue;

        if (spec.AnimationName == "Idle" && !mobileIncludeIdle)
            continue;
...
AppendMostRecentExportedAssetPaths(
    MobileTownExteriorFolderAssetRelative.TrimEnd('/') + "/" + entityName);
...
AppendMostRecentExportedAssetPaths(
    rootAssetRelative.TrimEnd('/') + "/" + entityName);
...
{
    return "NexusDev.ExportDaggerfallAssets_West.MobileLastDurationSeconds." + operationName;
}
...
{
    ExportAllFixedCategories(true);
}

private void ExportAllFixedCategories(bool beginTrackingMostRecentOperation)
{
    ...
    ApplySharedModelBatchTimingOverhead(
        new List<string>(FixedCategoryBatchSources),
        (DateTime.UtcNow - batchStartUtc).TotalSeconds);
}
...
private void LoadPrefs()
{
    ...
    atlasBatchExportedMats = EditorPrefs.GetBool(PrefAtlasBatchExportedMats, true);
    ...
    categoryBatchExpanded = EditorPrefs.GetBool(PrefCategoryBatchExpanded, true);
    flatCategoryBatchExpanded = EditorPrefs.GetBool(PrefFlatCategoryBatchExpanded, true);
    singleModelSectionExpanded = EditorPrefs.GetBool("NexusDev.ExportDaggerfallAssets_West.SingleModelSectionExpanded", true);
    singleFlatSectionExpanded = EditorPrefs.GetBool("NexusDev.ExportDaggerfallAssets_West.SingleFlatSectionExpanded", true);
    ...
    blenderImportScriptExpanded = EditorPrefs.GetBool("NexusDev.ExportDaggerfallAssets_West.BlenderImportScriptExpanded", false);
...
private void SavePrefs()
{
    ...
    EditorPrefs.SetBool("NexusDev.ExportDaggerfallAssets_West.BlenderImportScriptExpanded", blenderImportScriptExpanded);
    EditorPrefs.SetBool("NexusDev.ExportDaggerfallAssets_West.SingleModelSectionExpanded", singleModelSectionExpanded);
    EditorPrefs.SetBool("NexusDev.ExportDaggerfallAssets_West.SingleFlatSectionExpanded", singleFlatSectionExpanded);
...
private void ExportSingleConfiguredModel()
{
    ...
    try
    {
        if (atlasBatchExportedMats)
        {
            int sharedSuccess;
            int sharedFail;
            ExportIdsSharedAtlasWithBaseName(ids, BuildBatchBaseName(ids), null, out sharedSuccess, out sharedFail);
            successCount += sharedSuccess;
            failCount += sharedFail;
        }
        else
        {
            for (int i = 0; i < ids.Count; i++)
            {
                ...
                ResolveModelExportNaming(ids[i], out exportBaseName, out categoryFieldNameOrNull);
...
private void ExportSingleConfiguredFlat()
{
    ...
    try
    {
        if (atlasBatchExportedMats)
        {
            string sharedBatchAssetBaseName;

            if (requests.Count == 1)
            {
                sharedBatchAssetBaseName = BuildResolvedExportBaseNameForFlat(
                    requests[0].FlatArchive,
                    requests[0].FlatRecord);
            }
            else
            {
                sharedBatchAssetBaseName =
                    "FlatBatch_id" +
                    requests[0].FlatArchive.ToString(CultureInfo.InvariantCulture) + "_" +
                    requests[0].FlatRecord.ToString(CultureInfo.InvariantCulture) +
                    "_to_" +
                    requests[requests.Count - 1].FlatArchive.ToString(CultureInfo.InvariantCulture) + "_" +
                    requests[requests.Count - 1].FlatRecord.ToString(CultureInfo.InvariantCulture);
            }

            ExportFlatRequestsSharedAtlasWithBaseName(
                requests,
                sharedBatchAssetBaseName,
                null,
                out successCount,
        ...
        else
        {
            for (int i = 0; i < requests.Count; i++)
            {
                ...
                string exportBaseName;
                string categoryFieldNameOrNull;
                ResolveFlatExportNaming(
                    request.FlatArchive,
                    request.FlatRecord,
                    out exportBaseName,
                    out categoryFieldNameOrNull);
    ...
    Debug.Log(string.Format(
        CultureInfo.InvariantCulture,
        "[NexusDev][DFU Export] Single flat export finished. Success: {0}, Failed: {1}",
        successCount,
        failCount));
}
    ...
    try
    {
        ...
        for (int i = 0; i < expandedRequests.Count; i++)
        {
            ...
            if (request.IsFlat)
            {
                string exportBaseName;
                string categoryFieldNameOrNull;
                ResolveFlatExportNaming(request.FlatArchive, request.FlatRecord, out exportBaseName, out categoryFieldNameOrNull);
            ...
            else
            {
                string exportBaseName;
                string categoryFieldNameOrNull;
                ResolveModelExportNaming(request.ModelId, out exportBaseName, out categoryFieldNameOrNull);
...
private void ExportSelectedCategoryBatches()
{
    ...
    ApplySharedModelBatchTimingOverhead(
        new List<string>(selectedCategoryBatchSources),
        (DateTime.UtcNow - batchStartUtc).TotalSeconds);
}
...
private void ExportSelectedFlatCategoryBatches()
{
    ...
    ApplySharedFlatBatchTimingOverhead(
        new List<string>(selectedFlatCategoryBatchSources),
        (DateTime.UtcNow - batchStartUtc).TotalSeconds);
}

private void ExportAllFlatCategories()
{
    ExportAllFlatCategories(true);
}
private void ExportAllFlatCategories(bool beginTrackingMostRecentOperation)
{
    string[] flatSources = GetAvailableFlatCategoryBatchSources();
    DateTime batchStartUtc = DateTime.UtcNow;
    ...
    out string error)
{
    ...
    try
    {
        ...
        AppendMostRecentExportedAssetPaths(exportedAssetPath);
        QueueDeferredFbxMaterialRemap(exportedAssetPath, mats);
    ...
    out string error)
{
    ...
    if (sourceGo == null)
    {
        error = "Source GameObject was null.";
        return false;
    }
    ...
    {
        error = "Source object had no mesh.";
        return false;
    }
    ...
    Dictionary<string, Texture2D> uniqueTextures = new Dictionary<string, Texture2D>(StringComparer.OrdinalIgnoreCase);
    Dictionary<string, BatchTexturePlacement> placements = null;
    List<BatchTexturePage> pages = null;
    Mesh remappedMesh = null;
    ...
    try
    {
        for (int i = 0; i < textureKeys.Length; i++)
        {
            ...
            if (string.IsNullOrWhiteSpace(key) || uniqueTextures.ContainsKey(key))
                continue;

            Texture2D tex;
            string textureError;
            if (!TryLoadDfuTexture(key, out tex, out textureError))
            {
                error = "Could not load texture '" + key + "'. " + textureError;
                return false;
            }
        ...
        {
            error = "No readable textures were resolved for model " + modelId.ToString(CultureInfo.InvariantCulture) + ".";
            return false;
        }

        string packError;
        if (!TryBuildSharedBatchAtlasPages(
            uniqueTextures,
            exportBaseName,
            batchAtlasMaxSize,
            out pages,
            ...
            out packError))
        {
            error = "Atlas build failed. " + packError;
            return false;
        }

        List<int> localPageIndices;
        remappedMesh = BuildSharedBatchAtlasedMesh(sourceMf.sharedMesh, textureKeys, placements, out localPageIndices);
        if (remappedMesh == null)
        {
            error = "Failed to build atlased export mesh.";
            return false;
        }
        ...
        for (int i = 0; i < localPageIndices.Count; i++)
        {
            int pageIndex = localPageIndices[i];

            if (pages == null || pageIndex < 0 || pageIndex >= pages.Count || pages[pageIndex] == null || pages[pageIndex].MaterialAsset == null)
            {
                error = "Atlas page/material lookup failed while building export object.";
                DestroyImmediate(exportGo);
                exportGo = null;
                return false;
            }

            mats[i] = pages[pageIndex].MaterialAsset;
        }

        outMr.sharedMaterials = mats;
        return true;
    }
    catch (Exception ex)
    {
        ...
        error = "Exception while building atlased export object. " + ex;
        return false;
    }
    finally
    {
        ...
        if (pages != null)
        {
            for (int i = 0; i < pages.Count; i++)
            {
                if (pages[i] != null && pages[i].AtlasTexture != null)
                    DestroyImmediate(pages[i].AtlasTexture);
            }
        }
    }
}
...
private bool ExportOneModel(uint modelId, string exportBaseName, string categoryFieldNameOrNull, bool useCategorySubfolder)
{
    ...
    if (string.IsNullOrEmpty(exportBaseName))
        ResolveModelExportNaming(modelId, out exportBaseName, out categoryFieldNameOrNull);
    ...
    try
    {
                if (ShouldSkipSingleExportBecauseAssetsExist(
            exportBaseName,
            categoryFieldNameOrNull,
...
AppendMostRecentExportedAssetPaths(
    exportedAssetPath,
    BuildSingleAtlasAssetRelativePath(exportBaseName),
    BuildSingleMaterialAssetRelativePath(exportBaseName));
...
private bool ExportOneFlat(int archive, int record, string exportBaseName, string flatGroupNameOrNull, bool useFlatGroupSubfolder)
{
    ...
    if (string.IsNullOrEmpty(exportBaseName))
        ResolveFlatExportNaming(archive, record, out exportBaseName, out flatGroupNameOrNull);
    ...
    try
    {
        ...
        try
        {
            ...
            bool pausedForImmediateImports = PauseDeferredAssetEditingForImmediateImports();
            try
            {
                AssetDatabase.ImportAsset(
                    tempFlatTextureAssetRelative,
    ...
    materialAssetRelative);
            ...
            {
                ResumeDeferredAssetEditingAfterImmediateImports(pausedForImmediateImports);
            }
        }
        finally
        {
    ...
    out string error)
{
    ...
    if (string.IsNullOrWhiteSpace(exportBaseName) || string.IsNullOrWhiteSpace(categoryFieldNameOrNull))
    {
        string resolvedExportBaseName;
        string resolvedCategoryField;
        ResolveModelExportNaming(modelId, out resolvedExportBaseName, out resolvedCategoryField);

        if (string.IsNullOrWhiteSpace(exportBaseName))
            exportBaseName = resolvedExportBaseName;

        if (string.IsNullOrWhiteSpace(categoryFieldNameOrNull))
            categoryFieldNameOrNull = resolvedCategoryField;
    }
...
{
    string folderName = string.IsNullOrWhiteSpace(flatGroupName)
        ? "FlatGroup"
        : flatGroupName.Trim();
...
private bool ShouldSkipSingleExportBecauseAssetsExist(
    string exportBaseName,
    string categoryFieldNameOrNull,
    bool useCategorySubfolder)
{
...
private void ApplySharedModelBatchTimingOverhead(List<string> fieldNames, double totalBatchSeconds)
{
    if (fieldNames == null || fieldNames.Count == 0 || totalBatchSeconds <= 0d)
        return;
    ...
    for (int i = 0; i < uniqueFieldNames.Count; i++)
    {
        ...
        string key = fieldName + (exportGroupedFamilyMembers ? "|Grouped" : "|Base");
        keys.Add(key);

        double seconds;
        if (categoryLastDurationSeconds.TryGetValue(key, out seconds) && seconds > 0d)
            measured += seconds;
    }

    double overhead = totalBatchSeconds - measured;
    if (overhead <= 0d)
        return;

    double addPerCategory = overhead / uniqueFieldNames.Count;
    ...
    for (int i = 0; i < uniqueFieldNames.Count; i++)
    {
        ...
        double current = 0d;
        categoryLastDurationSeconds.TryGetValue(key, out current);
        current += addPerCategory;
        categoryLastDurationSeconds[key] = current;
...
private void ApplySharedFlatBatchTimingOverhead(List<string> fieldNames, double totalBatchSeconds)
{
    if (fieldNames == null || fieldNames.Count == 0 || totalBatchSeconds <= 0d)
        return;

    HashSet<string> unique = new HashSet<string>(fieldNames, StringComparer.OrdinalIgnoreCase);
    if (unique.Count == 0)
        return;
    ...
    foreach (string fieldName in unique)
    {
        string key = "FlatCategory|" + fieldName + (exportGroupedFamilyMembers ? "|Grouped" : "|Base");
        keys.Add(key);
    ...
    double addPerCategory = overhead / unique.Count;
...
private void DeleteAllExportedEverything()
{
    try
    {
        DeleteAllExportedAssets();
        ...
        Debug.Log("[NexusDev][DFU Export] ULTRA DELETE completed for Models, Flats, and MobileSprites.");
    }
    catch (Exception ex)
    {
        Debug.LogError("[NexusDev][DFU Export] ULTRA DELETE failed. Exception: " + ex);
    }
}
...
private static void EnsureFolderExists(string assetRelativeFolder)
{
    ...
    string normalizedInput = assetRelativeFolder.Replace('\\', '/').TrimEnd('/');
    if (string.IsNullOrEmpty(normalizedInput))
        return;

    if (!normalizedInput.StartsWith("Assets", StringComparison.OrdinalIgnoreCase))
    {
        string absNonAsset = GetAbsolutePathFromAssetPath(normalizedInput);
        if (!string.IsNullOrEmpty(absNonAsset) && !Directory.Exists(absNonAsset))
            Directory.CreateDirectory(absNonAsset);
        return;
    }

    if (ensuredFolderCache.Contains(normalizedInput))
        return;

    if (AssetDatabase.IsValidFolder(normalizedInput))
    {
        ensuredFolderCache.Add(normalizedInput);
        return;
    }
    ...
    ensuredFolderCache.Add(current);

    for (int i = 1; i < parts.Length; i++)
    {
        string next = current + "/" + parts[i];

        if (!ensuredFolderCache.Contains(next))
        {
            if (!AssetDatabase.IsValidFolder(next))
                AssetDatabase.CreateFolder(current, parts[i]);

            ensuredFolderCache.Add(next);
        }
...
private static void DeleteEmptyNumberedDuplicateFoldersUnder(string rootAssetFolder)
{
    string rootAbs = GetAbsolutePathFromAssetPath(rootAssetFolder);
    if (string.IsNullOrEmpty(rootAbs) || !Directory.Exists(rootAbs))
        return;

    string[] dirs = Directory.GetDirectories(rootAbs, "*", SearchOption.TopDirectoryOnly);
    if (dirs == null || dirs.Length == 0)
        return;

    HashSet<string> existingNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
    for (int i = 0; i < dirs.Length; i++)
        existingNames.Add(Path.GetFileName(dirs[i]));

    for (int i = 0; i < dirs.Length; i++)
    {
        string absDir = dirs[i];
        ...
        string canonicalName = match.Groups[1].Value;
        ...
        bool hasFiles = Directory.GetFiles(absDir, "*", SearchOption.AllDirectories).Length > 0;
        bool hasSubdirs = Directory.GetDirectories(absDir, "*", SearchOption.AllDirectories).Length > 0;
        if (hasFiles || hasSubdirs)
            continue;

        string assetPath = rootAssetFolder.TrimEnd('/') + "/" + name;
        FileUtil.DeleteFileOrDirectory(assetPath);
        FileUtil.DeleteFileOrDirectory(assetPath + ".meta");
    }
}

private static void DeleteKnownEmptyDuplicateExportFolders()
{
    DeleteEmptyNumberedDuplicateFoldersUnder(FbxModelFolderAssetRelative);
    DeleteEmptyNumberedDuplicateFoldersUnder(FbxFlatFolderAssetRelative);
}
